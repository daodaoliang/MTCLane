...
public class MyComm implements SerialPortEventListener {
    ...
    private final ConcurrentLinkedQueue<byte[]> sendQueue = new ConcurrentLinkedQueue<byte[]>();
    private final ConcurrentLinkedQueue<Byte> recvQueue = new ConcurrentLinkedQueue<Byte>();
    ...
    private static final Logger logger = Logger.getLogger(MyComm.class);
    private final int maxQueueSize = 1024;//队列最大值
    private final Object sendObj = new Object();//用于对发送队列加锁的对象
    private final Object recvObj = new Object();//用于对接收队列加锁的对象
...
//                logger.debug("recvQueue check start");
                synchronized (recvObj) {
