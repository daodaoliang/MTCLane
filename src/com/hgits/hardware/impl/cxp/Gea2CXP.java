 ...
 * gea协议且兼容意丰汇智cxp板接入类
 *
 * @author zengzb 2016年10月19日 下午2:54:43
 */
public class Gea2CXP implements SerialPortEventListener, CXP {

    private final static String REPLY_ORDER = "0A410D";//应答命令。
...
//                while (sbRead.indexOf("0A") != -1) {
    ...
    private void checkHasCar(String readStr) {
        ...
        // 长指令判断线圈是否感应到车
        if (readStr.contains("300D")) {// 无车状态
            write("主机答应", "0A410D");
            reachCoilFlag = false;
            channelCoilFlag = false;
        }
        if (readStr.contains("310D")) {//后线圈有车
            write("主机答应", "0A410D");
            reachCoilFlag = false;
            channelCoilFlag = true;
        }

        if (readStr.contains("340D")) {//前线圈有车
            write("主机答应", "0A410D");
            reachCoilFlag = true;
            channelCoilFlag = false;
        }
        if (readStr.contains("350D")) {//前后线圈同时有车
            write("主机答应", "0A410D");
            reachCoilFlag = true;
            channelCoilFlag = true;
        }
