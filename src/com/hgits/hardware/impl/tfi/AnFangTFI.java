 ...
 *
 */
public class AnFangTFI implements TFI {
...
//            loadOrder(order);
    ...
    /**
     * 针对江苏安防费显只能单行显示的问题，编写此方法
     */
    private void showAll() {
        byte[] buff = null;
        byte[] tempBuffer = null;//用于临时保存需要显示的字节数据
        try {
            for (int row = 1; row <= 4; row++) {
                ...
                System.arraycopy(tempBuffer, 0, buff, 0, 16);
                showLine(new String(buff, "gbk"), row);
