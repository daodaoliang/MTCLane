...
public class GeaLprService1 implements Runnable, LprService {
    ...
    //默认的车牌号码的正则表达式
    private final String DEFAULT_PLATENUM_REG = "^[京渝沪津湘皖闽甘云贵琼冀黑豫鄂苏赣吉辽青陕鲁晋川粤浙桂蒙宁藏新警消边广北南山兰成济军海空港澳使领境测挂水电林试临学通A-Z]{1}[A-Z0-9]{1}[京渝沪津湘皖闽甘云贵琼冀黑豫鄂苏赣吉辽青陕鲁晋川粤浙桂蒙宁藏新警消边广北南山兰成济军海空港澳使领境测挂水电林试临学通]{0,1}[A-Z0-9]{4}[A-Z0-9警消边广北南山兰成济军海空港澳使领境测挂水电林试临学通超]{1}$";
    private int lprPlateCnt = 100;

    public GeaLprService1(CXP cxp) {
    }
    ...
    private void init() {
        ...
        str = MyPropertiesUtils.getProperties(Constant.PROP_MTCLANE_CONSTANT, "lprPlateCnt", "100");
        int i = IntegerUtils.parseString(str);
        if (i <= 0) {
            i = 100;
        }
        lprPlateCnt = i;
        MTCLog.log("LPR:lprPlateCnt=" + lprPlateCnt);
    }
...
//                if (lprPlateMode == 0) {//循环抓拍模式判断才会进行车牌号码有效性的判断
                if (tempCnt < lprPlateCnt && lastPlateNum != null && lastPlateNum.equalsIgnoreCase(str1)) {//当前车牌与上一辆车的车牌相同（三次之内）
                    MTCLog.log("LPR:识别车牌号码" + str1 + "与上一辆车" + lastPlateNum + "相同，第" + tempCnt + "次");
                    if (lprPlateCnt < 100) {
                        tempCnt++;
                    }
                    continue;
                ...
                if (str1 == null || str2 == null
                        ...
                        || str2.trim().equals("_") || str1.trim().isEmpty() || str2.trim().isEmpty()) {
                    MTCLog.log("LPR:识别车牌颜色" + str2 + "，识别车牌号码" + str1 + "不符合验证规则");
                    continue;
                } else if (!checkPlateNum(str1)) {
                    MTCLog.log("LPR:识别车牌号码" + str1 + "不符合验证规则");
                    continue;
                }
//                }
//                lastPlateNum = str1;
...
//                FlowControl.chargeObserCode = "W";
    ...
    private boolean checkPlateNum(String plateNum) {
        ...
        String reg = MyPropertiesUtils.getProperties(Constant.PROP_MTCLANE_CONSTANT, "lprPlateReg", DEFAULT_PLATENUM_REG);
        if (plateNum.matches(reg)) {
            flag = true;
        }
        return flag;
    }
