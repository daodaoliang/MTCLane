 ...
 * @author Wang Guodong
 */
public class TempServiceNew implements TempServiceInterface {

    private static TempServiceNew instance;//唯一实例化对象
     ...
     * @return 唯一实例化对象
     */
    public static synchronized TempServiceNew getSingleInstance() {
        if (instance == null) {
            instance = new TempServiceNew();
        }
        return instance;
    }
    ...
    private void init() {
        ...
        ThreadPoolControl.getThreadPoolInstance().execute(tempTask);
    }

    private TempServiceNew() {
        init();
    }
    ...
    private void generateTemp(String str, String path) {
        ...
        sb.append(path).append(spliteMark).append(str);
        tempQueue.add(sb.toString());
    }
     ...
     * @param path 临时文件路径(包含文件名)
     */
    private void recordTemp(String str, String path) {
        ...
        try {
            ...
            if (str == null || str.trim().isEmpty()) {
                logger.debug("清空临时文件：" + path);
                fc.truncate(0);
            } else {
                logger.debug("更新临时文件：" + path + ",内容：" + str);
    ...
    private void truncateFile(File file) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append(path).append(spliteMark).append(" ");
        tempQueue.add(sb.toString());
    }
    ...
    private final String spliteMark = ",";
    ...
    class TempTask extends MyTask {

        @Override
        public void run() {
            Thread.currentThread().setName("TempFileRecordThread");
            while (running) {
                try {
                    Thread.sleep(1);
                } catch (InterruptedException ex) {
                }
                String str = tempQueue.poll();
                if (str != null) {
                    String[] buffer = StringUtils.split(str, spliteMark, 2);
                    if (buffer.length != 2) {
                        logger.debug("发现异常临时流水记录任务,需要记录的临时流水为" + str);
                        continue;
                    }
                    recordTemp(buffer[1].trim(), buffer[0]);
                }
            }
        }
