    ...
    class SendTask extends MyTask {
        ...
        public void run() {
            try {
                ...
                setAlive(true);
                //modify by wanghao 第一次启动通讯的时候，把车道状态发送到服务器。
                String str = firstMessage();
                sendInfoMessage(str);

//                String str = HeartBeatCommand + getSerial() + FlowControl.getCanopyFlag();//指令内容由指令标识+指令序列号+指令内容组成
//                send(str);
                ...
                while (running) {
                    ...
                    order = sendQueue.poll();//先进先出
                    if (order != null) {
                        if (logLevel >= 1) {
                            logger.warn("send to " + remoteAddress + ":" + order);
                        }
                        buffer = (str2HexStr(order) + "\r\n").getBytes();
                    ...
                    if (now < start) {//当前时间小于开始时间，重新开始计时
                        ...
                        start = now;
                        order = HeartBeatCommand + getSerial() + FlowControl.getCanopyFlag();//指令内容由指令标识+指令序列号+指令内容组成
                        send(order);
                    }
        ...
        /**
         * 第一次心跳消息，把车道收费员，雨棚灯状态、车道、出口站信息发到监控室
         *
         * @return
         */
        private String firstMessage() {
            ...
            if (lane.isEntry()) {//入口
                ...
                ei.setAlarmType("0");
                ei.setCanopyLight(String.valueOf(FlowControl.getCanopyFlag()));
                ei.setUserId(FlowControl.staff == null ? "" : FlowControl.staff.getStaffId());
                message = ei.toString();
                ...
                exi.setCanopyLight(String.valueOf(FlowControl.getCanopyFlag()));
                exi.setUserId(FlowControl.staff == null ? "" : FlowControl.staff.getStaffId());
                message = exi.toString();
            }
            return message;
        }
    }
            ...
            try {
                ...
                while (running) {
                    ...
                    if (len == -1) {
                        ...
                        tempBuffer = Arrays.copyOf(buffer, len);
                        if (logLevel >= 1) {
