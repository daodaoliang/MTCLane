...
public class MiFareService {
    ...
    public String openCard() {
        ...
        if (i == 0) {
            int infoLen = len[0];
            infoLen = infoLen >= info.length ? info.length : infoLen;
            byte[] buffer = Arrays.copyOf(info, infoLen);
            byte[] serialBuffer = new byte[infoLen];
            for (int j = 0; j < infoLen; j++) {
                serialBuffer[infoLen - j - 1] = buffer[j];
            }
            long serial = LongUtils.bytesToLong(serialBuffer);
            cardNo.append("0").append(",").append(StringUtil.toLength(serial, 10));
        } else {
     ...
     * @return true/false
     */
    public boolean openCard(String cardSerial) {
        if (cardSerial == null) {
            return false;
        }
        boolean flag = false;
        ...
        if (i == 0) {
            ...
            long serial = LongUtils.bytesToLong(serialBuffer);
            String strSerial = StringUtil.toLength(serial, 10);
            flag = cardSerial.equalsIgnoreCase(strSerial);
        } else {
            cardNo.append(i).append(", ");
        }
        return flag;
    ...
    public int authenticateWithKey(int keyType, int sector, String key, String cardid) {
        ...
        log("authenticateWithKey:用时" + (now - start) + "毫秒,result=" + i + ",keyType=" + keyType + ",sector=" + sector + ",key=" + ",cardid=" + cardid);
        return i;
    }
    ...
    public String readBlockEx(int block, int keyType, String key) {
        ...
        long now = System.currentTimeMillis();
        log("readBLockEx:用时" + (now - start) + "豪秒,i=" + i + ",block=" + block + ",keyType=" + keyType + ",key=" + ",info=" + new String(info));
        String str = null;
        if (i == 0) {
    ...
    public int writeBlockEx(int block, int keyType, String key, String data) {
        ...
        long now = System.currentTimeMillis();
        log("writeBlockEx:用时" + (now - start) + "毫秒，result=" + i + ",block=" + block + ",keyType=" + keyType + ",key="  + ",data=" + data);
        return i;
    }
     ...
     * 获取ETC卡余额，单位分
     *
     * @return 卡片余额，单位分；-1表示获取失败
     */
    public long getETCBalance() {
        byte[] resdata = new byte[128];
        byte[] sw12 = new byte[128];
        int i = etcRW.PRO_CommandEx(nPort, "00A40000021001", resdata, sw12);//打开DF目录
        if (i == 0) {
            MTCLog.log("打开DF目录指令执行成功,响应状态码为" + new String(sw12).trim() + ",响应帧为" + new String(resdata).trim() + "\n");
            resdata = new byte[128];
            sw12 = new byte[128];
            i = etcRW.PRO_CommandEx(nPort, "805C000204", resdata, sw12);//读取余额
            if (i == 0) {
                MTCLog.log("读取余额指令执行成功,响应状态码为" + new String(sw12).trim() + ",响应帧为" + new String(resdata).trim() + "\n");
                return Long.parseLong(new String(resdata).trim(), 16);
            } else {
                MTCLog.log("读取余额指令执行失败，返回结果" + i + "\n");
            }
        } else {
            MTCLog.log("打开DF目录指令执行失败，返回结果" + i + "\n");
        }
        return -1;
    }
