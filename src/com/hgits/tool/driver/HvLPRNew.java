 ...
 * @author lq
 */
public class HvLPRNew {

    private HvInterface hvdll;
    private int nPort;
    private String message = "";
    private static final Logger logger = Logger.getLogger(HvLPRNew.class.getName());
    ...
    private int lprPlateNumBufferSize = 20;//识别车牌缓冲区长度，单位byte
    private int lprPlateImgBufferSize = 50;//识别车牌缓冲区长度，单位k
     ...
     */
    private void init() {
        String str = MyPropertiesUtils.getProperties(Constant.PROP_MTCLANE_CONSTANT, "lprPlateNumBufferSize", "20");
        int i = IntegerUtils.parseString(str);
        if (i <= 0) {
            i = 20;
        }
        lprPlateNumBufferSize = i;
        LogControl.logInfo("lprPlateNumBufferSize=" + lprPlateNumBufferSize);
        str = MyPropertiesUtils.getProperties(Constant.PROP_MTCLANE_CONSTANT, "lprPlateImgBufferSize", "50");
        i = IntegerUtils.parseString(str);
        if (i <= 0) {
            i = 50;
        }
        lprPlateImgBufferSize = i;
        LogControl.logInfo("lprPlateImgBufferSize=" + lprPlateImgBufferSize);
    }
     ...
     */
    public synchronized int initHvLPR() {
...
//            }
     ...
     */
    public synchronized int closeHvLPR() {
        if (hvdll == null) {
            return -1;
        }
     ...
     * 车牌识别设备不能运行
     */
    public synchronized int lprIsReady() {
        if (hvdll == null) {
            return -1;
        }
     ...
     */
    public synchronized int captuerImage() {
     ...
     */
    public synchronized long getLastTimePlate() {
     ...
     */
    public synchronized String getPlate() {
        String str = "";
        byte[] lprPlateNum = new byte[lprPlateNumBufferSize];
        int i = hvdll.LPR_GetPlate(nPort, lprPlateNum, lprPlateNum.length);
        if (i > 0) {
            try {
                str = new String(lprPlateNum, "gbk").trim();
...
//                message = "车牌识别器未正常执行指令，连接可能存在异常。";
    ...
    public synchronized byte[] getPicture() {
        byte[] byteimg;
        byte[] info = new byte[lprPlateImgBufferSize * 1024];                          //先将传入的接收车图片的字节数组初始化为50K大小
        int[] j = new int[1];
...
//            byteimg = info;
            int len = j[0];//获取实际图片长度
            if (len > info.length) {//实际图片长度不能超过缓冲区长度
                len = info.length;
            }
            byteimg = Arrays.copyOf(info, len);
        } else {
            byteimg = null;
        }
        return byteimg;
     ...
     */
    public synchronized String getPlateColor() throws UnsupportedEncodingException {
        String str = "";
        byte[] info = new byte[1];
        int i = hvdll.LPR_GetPlateColor(nPort, info);
        if (i > 0) {
            str = new String(info, "gbk").trim();
        } else if (i == 0 || i < 0) {
            str = null;
        }
        return str;
     ...
     */
    public synchronized byte[] getPlatePicture(StringBuffer lpr) throws UnsupportedEncodingException {
        byte[] byteimg = new byte[lprPlateImgBufferSize * 1024];
        byte[] infoPlate = new byte[lprPlateNumBufferSize];
        byte[] infoImg = new byte[lprPlateImgBufferSize * 1024];
        ...
        if (i > 0) {
            if (i == 1) {
                str = new String(infoPlate, "gbk").trim();
