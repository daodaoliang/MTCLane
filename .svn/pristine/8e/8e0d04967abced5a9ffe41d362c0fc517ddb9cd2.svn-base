...
public class ParamCacheQuery {
	...
	static ETCCardBlackControl etcCardBlackControl = ETCCardBlackControl.getInstance();
    ...
    public static XTCardBlackList queryXTCardBlackList(String xtCardNo) {
		...
		if(etcCardBlackControl.isOptimisedActive()){
			xtCardBlackList = etcCardBlackControl.queryETCCardBlackList(xtCardNo);
    	}else{
            ...
            if (xtCardBlackDeltaList == null) {
                if (XTCardBlackBaseListByte.getBuffer7() != null) {
                    xtCardBlackDeltaList = check(XTCardBlackBaseListByte.getBuffer7(),
                            searchXTCardNoByte, true);
                }
            }
    ...
    public static XTCardBlackDeltaList check(byte[] buffer,
        ...
        if (isAll) { // 全量表查找
            ...
            byte[] empty = new byte[length]; // ETC卡黑名单空记录长度
            ...
            for (int i = 0; i < size;) {
				...
				if (Arrays.hashCode(record) == Arrays.hashCode(empty)) { // 没有数据了
					break;
				}
            ...
            for (int i = 0; i < size;) {
                System.arraycopy(buffer, i, cardBuffer, 0, 20);
                System.arraycopy(buffer, i, record, 0, length);
                if(Arrays.hashCode(record) == Arrays.hashCode(empty)){ //数据为空
					 break;
                }
                if (Arrays.equals(cardBuffer, searchXTCardNo)) {
                    xtCardBlackDeltaList = new XTCardBlackDeltaList();
    ...
    private static XTCardBlackDeltaList queryXTCardBlackDeltaList(
        ...
        if (xtCardBlackDeltaList == null) {
            if (XTCardBlackDeltaListByte.getBuffer7() != null) {
                xtCardBlackDeltaList = check(
                        XTCardBlackDeltaListByte.getBuffer7(),
                        searchXTCardNoByte, false);
            }
        }
...
//    }
    ...
    /**
     * 是否为公务卡
     * @param officialCardNo 公务卡卡号
     */
	public static OfficialCard queryOfficialCard(String officialCardNo) {
		OfficialCard officialCard = null;
		Vector<OfficialCard> officialCards = ParamCache.getOfficialCardMap();
		for (OfficialCard officialCard2 : officialCards) {
			if (officialCard2.getOfficilCardNo().equalsIgnoreCase(
					officialCardNo)) {
				officialCard = officialCard2;
				break;
			}
		}
		return officialCard;
    ...
    public static void main(String[] args) {
		try {
			...
			XTCardBlackList xTCardBlackList = ParamCacheQuery
					.queryXTCardBlackList("43011339220100002222");
//			// .queryXTCardBlackList("43011510220500001206");
//			// // .queryXTCardBlackList("13011339220100002222");
...
//			OfficialCard officialCard = ParamCacheQuery.queryOfficialCard("205328");
//			System.out.println(officialCard);
