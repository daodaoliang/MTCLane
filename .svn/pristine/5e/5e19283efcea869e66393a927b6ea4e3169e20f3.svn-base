...
public class TimeCheckControl {
	 ...
	 * 配置提示所需的提醒时间间隔
	 */
	private static Long configDifference;
	...
	public static Long getConfigDifference() {
		configDifference = Long.valueOf(MyPropertiesUtils.getProperties(
				Constant.PROP_MTCLANE_FUNCTION, "checkTimeInterval", "8"));
	...
	public static boolean isVaildTime() {
		...
		long configDifference = getConfigDifference();
		BigDecimal bd = BigDecimal.valueOf(difference);
		bd = bd.divide(BigDecimal.valueOf(3600),2,BigDecimal.ROUND_HALF_UP).divide(BigDecimal.valueOf(1000),2,BigDecimal.ROUND_HALF_UP);
		double result  = bd.doubleValue(); //转换成小时
		//差异是否大于配置文件中设置的值
		if(Math.abs(result) >configDifference){
			return false;
		}
