...
public class GeaIcReaderWriter implements IcReaderWriter {
     ...
     * 读取卡的序列号，即防碰撞指令
     */
    public static final String ORDER_42 = "42 05 00 00 00 00 00";
    /**
     * 读写器是否初始化完成
     */
    ...
    public Card cardR;                  //读到的卡
    public Card cardW;                  //要写的卡
    ...
    private int readCard() {
        ...
        try {
            ...
            ParseUtil.parseCardInfo(cardR);
            i = irw.loadOrder(ORDER_45);
            if (i != 0) {
                return i;
            }
            if (cardR != null && cardR.getType().equals(Constant.CARD_TYPE_01)) {      //当只有是通行卡时才读取块7区 
//                boolean result2 = authenticateKey(cardNo, "02");    //发送用密钥存放区0的密钥A认证2扇区的命令
                boolean result2 = authKey(cardNo, "02");    //发送用密钥存放区0的密钥A认证2扇区的命令
                if (!result2) {
                    return -5;
                }
...
//                }
    ...
    private int writeCard() {
        int i = 0;
        try {
            if (cardR == null || !cardR.getCardSerial().
                    equals(cardW.getCardSerial())) {
                MTCLog.log("写卡时发现卡片与所读卡片不一致,读卡:" + cardR + " 写卡:" + cardW);
                return -1;
            }
            boolean flag;
            int cnt = 0;//写卡失败次数计数器
...
//                flag = authenticateKey(ParseUtil.unParseCardNo(cardW.getCardSerial()), "01");
                flag = authKey(ParseUtil.unParseCardNo(cardW.getCardSerial()), "01");
                if (!flag) {
                    return -5;
                }
                if (sec5Flag) {
...
//                flag = authenticateKey(ParseUtil.unParseCardNo(cardW.getCardSerial()), "02");
                flag = authKey(ParseUtil.unParseCardNo(cardW.getCardSerial()), "02");
                if (!flag) {
                    return -5;
                }
                if (sec8Flag) {
...
//                }
            ...
            //王国栋 2014-12-18修改，日志用MTCLOG记录
            MTCLog.log("M1卡天线写卡异常，写卡信息：" + cardW, ex);
            return -1;
        } finally {
            if (i != 0) {
                MTCLog.log("M1卡天线写卡失败，写卡信息：" + cardW);
            }
        }
        return i;
    }
     ...
     * @return 认证的结果
     */
    private boolean authKey(String cardNo, String section) {
        boolean flag = false;
        if (sendKeyOrder("400701" + cardNo + "00" + section)) {
            return true;
        }
        irw.loadOrder(ORDER_45);
        if (sendKeyOrder("400702" + cardNo + "04" + section)) {
            return true;
        }
        irw.loadOrder(ORDER_45);
        if (sendKeyOrder("400701" + cardNo + "01" + section)) {
            return true;
        }
        irw.loadOrder(ORDER_45);
        if (sendKeyOrder("400702" + cardNo + "05" + section)) {
            return true;
        }
        irw.loadOrder(ORDER_45);
        if (sendKeyOrder("400701" + cardNo + "02" + section)) {
            return true;
        }
        irw.loadOrder(ORDER_45);
        if (sendKeyOrder("400702" + cardNo + "06" + section)) {
            return true;
        }
        irw.loadOrder(ORDER_45);
        if (flag == false) {
    ...
    public int write(Card card, boolean sec5Flag, boolean sec6Flag, boolean sec8Flag, boolean sec9Flag) {
        ...
        while (true) {
            ...
            if (!hasCard || cardR == null) {//写卡时发现卡丢失
                MTCLog.log("icrw:写卡时发现卡已丢失");
                return 1;
            }
            if (now - start >= 3000) {
...
//                            resultKey1 = authenticateKey(cardNo, "01");   //发送用密钥存放区0的密钥A认证1扇区的命令
                            resultKey1 = authKey(cardNo, "01");   //发送用密钥存放区0的密钥A认证1扇区的命令
                            if (!resultKey1) {
                                continue Outer;
                            } else {
                                message = "";
