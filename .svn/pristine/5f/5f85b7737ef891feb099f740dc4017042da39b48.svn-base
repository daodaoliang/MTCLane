...
public class YhIOCTRL implements CXP{
	...
	private final static String DO_DEVICE_BJD = "BJD";//报警灯
	private final static String DO_DEVICE_YPD = "YPD";//雨蓬灯
	private final static String DO_DEVICE_TXD = "TXD";//通行灯
	private final static String DO_DEVICE_LGJ = "LGJ";//栏杆机
	private final static String DI_DEVICE_QXQ = "QXQ";//前线圈
	private final static String DI_DEVICE_HXQ = "HXQ";//后线圈
	private static Map<String,Integer> doDevicePos = null;//输出 设备位置定义集合
	private static Map<String,Integer> diDevicePos = null;//输入 设备位置定义集合
	...
	private int initiallize(){
		...
		//设置当前使用的设备
		if(null != instantDiCtrl && null != instantDoCtrl ){
			try {
	...
	private void sendOrderDO(String deviceTypeDO,String optType){
		...
		if(null != instantDoCtrl && StringUtils.isNotEmpty(instantDoCtrl.getSelectedDevice().Description)){
			try {			
				/*读取设备状态*/
				portData = readDO();
	...
	private byte[] readDI() throws Exception{
		...
		if(null != instantDiCtrl && StringUtils.isNotEmpty(instantDiCtrl.getSelectedDevice().Description)){
