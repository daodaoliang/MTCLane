...
public class MyCpuCardReader implements Runnable, CpuCardReader {
    ...
    private String current01KeyA;//已经经过验证的01区密钥A
    private String current01KeyB;//已经经过验证的01区密钥B
    private String current02KeyA;//已经经过验证的02区密钥A
    private String current02KeyB;//已经经过验证的02区密钥B
...
//                    m1CardR = null;
                    setCpuCardR(null);
                    setM1CardR(null);
                    current01KeyA = null;//重置当前卡的密钥信息
                    current01KeyB = null;//重置当前卡的密钥信息
                    current02KeyA = null;//重置当前卡的密钥信息
                    current02KeyB = null;//重置当前卡的密钥信息
                    hasCard = false;
                    cardIsRead = false;
                    continue;
                } else {
...
//        cpuCardW = null;
    ...
    private int writeBlock(int block, String data, String serial) {
        ...
        if (null != keya) {
            keyAList.remove(keya);
            keyAList.add(0, keya);//将已经验证过的密钥放在密钥集合首位
        }
        if (null != keyb) {
            keyBList.remove(keyb);
            keyBList.add(0, keyb);//将已经验证过的密钥放在密钥集合首位
        }
        int result = 0;
        //遍历A密钥，进行写卡
        ...
        for (int i = 0; i < aLen; i++) {
            ...
            if (result == 0) {
                ...
                if (!flag) {//打开失败
                    log("ETC:写卡" + serial + "块" + block + "失败后再次打开该卡失败");
                    return -1;
                }
            }
        }
        ...
        for (int i = 0; i < bLen; i++) {
            ...
            if (result == 0) {
                ...
                boolean flag = etcrw.openCard(serial);//打开指定序号的卡
                if (!flag) {//打开失败
                    log("写卡" + serial + "块" + block + "失败后再次打开该卡失败");
                    return -1;
                }
            }
        }
...
//                cpuCardR = null;
     ...
     * @return 写卡结果 0则成功，找不到所写卡返回1，返回其他写卡失败
     */
    public int write(CpuCard card) {
        ...
        while (true) {
            ...
            if (now - start >= 3000) { //2秒后还无法得到正确的结果则跳出循环
                readyToWrite = false;
...
//            m1CardR = null;
    ...
    private String readBlock(int block, String serial) {
        ...
        for (int i = 0; i < lenA; i++) {
            ...
            if (str != null) {
                //读卡成功，记录当前卡的密钥，便于后续操作
                if (block >= 4 && block <= 7) {//01扇区
                    current01KeyA = keyAList.get(i);
                } else if (block >= 8 && block <= 11) {//02扇区
                    current02KeyA = keyAList.get(i);
                }
                break;
            } else {
                //读卡失败，重置当前卡的密钥，便于后续操作
                if (block >= 4 && block <= 7) {//01扇区
                    current01KeyA = null;
                } else if (block >= 8 && block <= 11) {//02扇区
                    current02KeyA = null;
        ...
        for (int i = 0; i < lenB; i++) {
            ...
            if (str != null) {
                //读卡成功，记录当前卡的密钥，便于后续操作
                if (block >= 4 && block <= 7) {//01扇区
                    current01KeyB = keyBList.get(i);
                } else if (block >= 8 && block <= 11) {//02扇区
                    current02KeyB = keyBList.get(i);
                }
                break;
            } else {
                //读卡失败，重置当前卡的密钥，便于后续操作
