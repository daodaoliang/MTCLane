...
//                            LogControl.logInfo("ETC卡折扣率:" + rate);
    ...
    private double getTruckETCDisRate2() {
        String str = MyPropertiesUtils.getProperties(Constant.PROP_MTCLANE_FUNCTION, "truckETCDisRate2", "0.98").trim();
        double rate = DoubleUtils.parseString(str);
        return rate;
    }
...
//                            FlowControl.cardObserCode = "H";
            ...
            LogControl.logInfo("ETC卡黑名单检测通过");
            ...
            if (!TestControl.isVehDiffIgnored()) {//首先确认当前情况下需要判断车型车牌差异
                ...
                LogControl.logInfo("ETC卡车牌颜色，车牌号码检验通过");
            }
        } catch (Exception ex) {
...
//    }
    ...
    public CpuCard waitForCpuCard() {
        ...
        while (true) {
            ...
            extJFrame.updateInfo("", "请将ETC卡放置到天线上");
            try {
                Thread.sleep(500);
            } catch (InterruptedException ex) {
            }
    ...
    private boolean check0019(CpuCard card) {
        ...
        if (str == null) {//临时ETC信息为空
            ...
            if (buffer.length != 2) {//临时ETC信息格式错误
                LogControl.logInfo("临时ETC卡信息格式错误：" + str);
                flag = false;
            } else {
                String info0019 = buffer[1].trim();//获取所需验证的0019信息
                String str2 = card.getInfo19();
                LogControl.logInfo("记录0019：" + info0019);
                LogControl.logInfo("卡内0019：" + str2);
                if (str2 != null) {
                    str2 = str2.trim();
                } else {
                    return false;
                }
                int len1 = info0019.length();
                int len2 = str2.length();
                if (len1 > len2) {
                    flag = info0019.contains(str2);
                } else {
                    flag = str2.contains(info0019);
                }
//                flag = info0019.equalsIgnoreCase(str2);//与当前卡片的0019信息比对
                if (flag) {
                    LogControl.logInfo("ETC卡状态异常，但是0019文件比对一致，将当前的0019文件" + info0019 + "替换为0019文件" + buffer[0]);
    ...
    /**
     * 检验当前ETC卡的卡号与临时文件记录的ETC卡的卡号是否一致，如果一直，用临时文件中的0019文件内容替换当前卡中的0019文件
     *
     * @param card 当前ETC卡
     * @return true正常 false异常
     */
    private boolean check0019New(CpuCard card) {
        if (card == null) {
            return false;
        }
        ...
        String info0019 = card.getInfo19();
        boolean flag;
        TempControl tempSvc = TempControl.getSingleInstance();
        String str = tempSvc.parseTempEtcInfo();//获取ETC临时信息
        if (str == null) {//临时ETC信息为空
            ...
            if (buffer.length != 2) {//临时ETC信息格式错误
                ...
                String tempCardSerial = buffer[0].trim();//获取临时文件内记录的卡号
                String tempInfo0019 = buffer[1].trim();//获取临时文件内记录的0019信息
//                String str2 = card.getInfo19();
                LogControl.logInfo("记录临时ETC卡：" + tempCardSerial);
                LogControl.logInfo("需验证ETC卡：" + cardSerial);
                flag = cardSerial.equalsIgnoreCase(tempCardSerial);
                if (flag) {
                    LogControl.logInfo("ETC卡状态异常，但是卡号比对一致，将当前的0019文件" + info0019 + "替换为0019文件" + tempInfo0019);
                    card.setInfo19(tempInfo0019);
                    ParseUtil.pare19(card);
                    LogControl.logInfo("重新解析0019文件后获取ETC卡信息" + card);
                }
            }
