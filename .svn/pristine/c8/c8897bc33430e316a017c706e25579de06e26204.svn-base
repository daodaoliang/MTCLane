...
//        card.setSection_05_info(res.toString());
    ...
    public static String parse06(Card card) {
        ...
        String vehClass = doc.substring(14, 16);
        if (!checkVehClass(vehClass)) {
            logger.debug("解析入口车型" + vehClass + "异常");
            vehClass = "00";
        }
        card.setVehClass(vehClass);   //获取车型
...
//        }
    ...
    public static void pare15(CpuCard card) {
        ...
        String plateNum = "";
        try {
            plateNum = new String(HexUtils.decode(sb.substring(56, 80)), "gbk").trim();
        } catch (UnsupportedEncodingException ex) {
            logger.error(ex, ex);
        }
        ...
        if (card.getCardVersion().startsWith("4")) {//新国标ETC卡
            ...
            String vehClass = sb.substring(84, 86);

            int tempVehClass = IntegerUtils.parseHexString(vehClass);
            if (tempVehClass == 10 || tempVehClass > 20) {
                logger.debug("解析ETC卡0015文件发现车型记录为" + tempVehClass + "，未知车型，转换为0");
                tempVehClass = 0;
            } else if (tempVehClass > 10 && tempVehClass <= 20) {
                logger.debug("解析ETC卡0015文件发现车型记录为" + tempVehClass + "，根据国标转换为货车7型车");
                tempVehClass = 7;
            }
            card.setVehClass(tempVehClass + "");   //车型
//            if (!checkVehClass(vehClass)) {
//                logger.debug("解析车型" + vehClass + "异常");
//                vehClass = "00";
//            }
//            card.setVehClass(IntegerUtils.parseHexString(vehClass) + "");   //车型
...
//                String tempIssuer = card.getIssuer();
//                if (tempIssuer != null && tempIssuer.startsWith("C1FABDAD")) {//当前卡发行省份为“黑龙江”
...
//                    col = decodePlateCol(plateNum.substring(0, 1));
//                }
                String temp = plateNum.substring(1,2);
                if(temp.matches("[\u4e00-\u9fa5]")){//车牌号码第二位为中文表示第一位的黑为车牌颜色
                    col = decodePlateCol(plateNum.substring(0, 1));
                }
            } else {//车牌号码不以“黑”开头
                col = decodePlateCol(plateNum.substring(0, 1));
    ...
    public static void pare19(CpuCard card) {
        ...
        String roadid = sb.substring(10, 12);
        if (!checkRoadid(roadid)) {
            logger.debug("解析入口路段" + roadid + "异常");
            roadid = "00";
        }
        card.setRoadid(roadid);         //入口路段,注：此处不需要进行16进制转换
        String stationid = sb.substring(12, 14);
        if (!checkStationid(stationid)) {
            logger.debug("解析入口收费站" + stationid + "异常");
            stationid = "00";
        }
        card.setStationid(stationid);     //入口站,注：此处不需要进行16进制转换
...
//        Date date = new Date(LongUtils.parseHexString(time) * 1000);
        ...
        if (sb.length() >= 86) {
            ...
            if (!checkRoadid(exitRoadId)) {
                logger.debug("解析出口路段" + exitRoadId + "异常");
                exitRoadId = "00";
            }
            if (!checkStationid(exitStationId)) {
                logger.debug("解析出口收费站" + exitStationId + "异常");
                exitStationId = "00";
            }
    ...
    /**
     * 根据ETC卡内容获取0019文件6-43字节内容
     *
     * @param cpuCard CPU卡
     * @return 0019文件6-43字节内容，16进制
     */
    public static String unParse19(CpuCard cpuCard) {
        CpuCard tempCard = new CpuCard(cpuCard);
        String info19 = tempCard.getInfo19();
        if (info19 == null) {
            info19 = StringUtils.toLength("0", 86);
        }
        while (info19.length() <= 86) {
            info19 = info19 + "0";
        }
        info19 = info19.substring(0, 86);
        StringBuilder sb = new StringBuilder(info19);
...
//            data = StringUtils.toLength(data, 30);
     ...
     * @return true/false
     */
    private static boolean checkVehClass(String vehClass) {
        if (vehClass == null) {
            return false;
        }
        String reg = "0[1-9]";
        return vehClass.matches(reg);
    }
     ...
     * @return 16进制员工号
     */
    private static String staffidToHex(String staffid, int len) {
